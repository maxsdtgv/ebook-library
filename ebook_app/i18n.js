...
const TRANSLATIONS = {
    en: {
        ...
        'manage.exportJson': 'Export JSON copy',
        'manage.saveHint': 'Save writes <code>ebook_app/data.js</code>, which the app loads automatically next time — in Chrome/Edge you can overwrite the existing file in place. A browser cannot write to disk on its own, so this click is the one manual step. The JSON copy is just for backups.',
    ...
    ru: {
        ...
        'manage.exportJson': 'Выгрузить копию JSON',
        'manage.saveHint': 'Сохранение пишет <code>ebook_app/data.js</code>, который загружается автоматически при следующем запуске — в Chrome/Edge можно перезаписать существующий файл на месте. Браузер не может писать на диск сам, поэтому это нажатие — единственное ручное действие. Копия в JSON нужна только для резервных копий.',
        'manage.books': 'Книги',
